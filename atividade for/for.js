...
//Questão 21
let numeral = prompt("Digite um número inteiro");
if (numeral !== null) {
    let lista = numeral.split("");
    if (lista.length >= 2) {
        let primeiroDigito = Number(lista[0]);
        let segundoDigito = Number(lista[1]);
        let somando = primeiroDigito + segundoDigito;
...
let mediaAltura = totalAltura / numeroDePessoas;
...
let numeroStr = prompt("Digite um número inteiro:");
if (numeroStr !== null) {
    let numero = numeroStr;
    let soma = 0;
    for (let z = 0; z < numero.length; z++) {
        let digito = parseInt(numero[z], 10);
        if (digito % 2 === 0) {
            soma += digito;
        }
    }
...
//Questão 26
let numerop = prompt("Digite um número inteiro:");
if (numerop !== null) {
    let numeroInvertido = numerop.split("").reverse().join("");
